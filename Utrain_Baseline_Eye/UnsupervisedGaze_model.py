...
class GazeRepresentationLearning_halfface(nn.Module): #checked
    def __init__(self, channels=64) -> None:
        super(GazeRepresentationLearning_halfface, self).__init__()
        self.ConvBlock_1 = ConvBlock(input_channel=channels, last=True)
        self.ConvBlock_2 = ConvBlock(input_channel=channels)
        self.ConvBlock_3 = ConvBlock(input_channel=2*channels, last=True)
        self.ConvBlock_4 = ConvBlock(input_channel=2*channels)
        self.features= nn.Sequential(
            ...
            nn.AvgPool2d(kernel_size=(2,3), stride=(2,3)), # 1/3 1/4 for EyeDiap
            nn.Flatten(),
            nn.Linear(in_features=1024, out_features=channels//4),
            nn.Linear(in_features=channels//4, out_features=2),
        )
        self.channels = channels
    def forward(self, input):
...
class GazeRepresentationLearning_fullface_KD(nn.Module): #checked
    def __init__(self, channels=64) -> None:
        super(GazeRepresentationLearning_fullface_KD, self).__init__()
        self.ConvBlock_1 = ConvBlock(input_channel=channels, last=True)
        self.ConvBlock_2 = ConvBlock(input_channel=channels)
        self.ConvBlock_3 = ConvBlock(input_channel=2*channels, last=True)
        self.ConvBlock_4 = ConvBlock(input_channel=2*channels)
    ...
    def forward(self, input):
        ...
        feature_1 = input.to(self.device)
        input = self.features_2(input)
        feature_2 = input.to(self.device)
        input = self.features_3(input)
        feature_3 = input.to(self.device)
        input = self.features_4(input)
        feature_4 = input.to(self.device)
